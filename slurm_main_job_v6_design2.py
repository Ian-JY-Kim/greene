...
MC_master_data = pd.read_csv("MC_master_data_design2.csv")
...
for job_num in range(start_num, end_num+1):
    ...
    def f1_cost_shock_prob(shock, xi): #shock: -0.05 or 0.35
        z = (xi-0.1)/0.5
        denominator = 1 + np.exp(z)
        if shock == -0.05:
            prob = 1/denominator
        if shock == 0.35:
            prob = np.exp(z)/denominator
        return prob
    vec_f1_cost_shock_prob = np.vectorize(f1_cost_shock_prob)

    def f2_cost_shock_prob(shock, xi): #shock: -0.05 or 0.35
        z = xi/0.5
        denominator = 1 + np.exp(z)
        if shock == -0.05:
            prob = 1/denominator
        if shock == 0.35:
            prob = np.exp(z)/denominator
        return prob
    vec_f2_cost_shock_prob = np.vectorize(f2_cost_shock_prob)
    ...
    def EXP_pi_duo_f1_trapz(x_1, x_2, w_1, w_2, xi_1, xi_2_star):
        ...
        trapz_fun1 = f1_cost_shock_prob(-0.05, xi_1) * vec_f2_cost_shock_prob(0.35, xi_2_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1, xi_2_vec,-0.05, 0.35)[0] * norm.pdf(xi_2_vec, (xi_1-0.1)/2, np.sqrt(3)/4)
        trapz_fun2 = f1_cost_shock_prob(-0.05, xi_1) * vec_f2_cost_shock_prob(-0.05, xi_2_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1, xi_2_vec,-0.05,-0.05)[0] * norm.pdf(xi_2_vec, (xi_1-0.1)/2, np.sqrt(3)/4)
        trapz_fun3 = f1_cost_shock_prob( 0.35, xi_1) * vec_f2_cost_shock_prob( 0.35, xi_2_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1, xi_2_vec, 0.35, 0.35)[0] * norm.pdf(xi_2_vec, (xi_1-0.1)/2, np.sqrt(3)/4)
        trapz_fun4 = f1_cost_shock_prob( 0.35, xi_1) * vec_f2_cost_shock_prob(-0.05, xi_2_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1, xi_2_vec, 0.35,-0.05)[0] * norm.pdf(xi_2_vec, (xi_1-0.1)/2, np.sqrt(3)/4)
    ...
    def EXP_pi_duo_f2_trapz(x_1, x_2, w_1, w_2, xi_1_star, xi_2):
        ...
        vec_duo = np.vectorize(duopoly)
        trapz_fun1 = f2_cost_shock_prob( 0.35, xi_2) * vec_f1_cost_shock_prob(-0.05, xi_1_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1_vec, xi_2,-0.05, 0.35)[1] * norm.pdf(xi_1_vec, 0.1+(xi_2/2), np.sqrt(3)/4)
        trapz_fun2 = f2_cost_shock_prob(-0.05, xi_2) * vec_f1_cost_shock_prob(-0.05, xi_1_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1_vec, xi_2,-0.05,-0.05)[1] * norm.pdf(xi_1_vec, 0.1+(xi_2/2), np.sqrt(3)/4)
        trapz_fun3 = f2_cost_shock_prob( 0.35, xi_2) * vec_f1_cost_shock_prob( 0.35, xi_1_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1_vec, xi_2, 0.35, 0.35)[1] * norm.pdf(xi_1_vec, 0.1+(xi_2/2), np.sqrt(3)/4)
        trapz_fun4 = f2_cost_shock_prob(-0.05, xi_2) * vec_f1_cost_shock_prob( 0.35, xi_1_vec) * vec_duo(x_1, x_2, w_1, w_2, xi_1_vec, xi_2, 0.35,-0.05)[1] * norm.pdf(xi_1_vec, 0.1+(xi_2/2), np.sqrt(3)/4)
